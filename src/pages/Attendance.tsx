...
import { Calendar, Clock, Plus, Filter, Users, CheckCircle, XCircle, LogIn, User, Eye, Edit, History } from "lucide-react";
...
const AttendanceCard = ({ record, onEdit, onViewProfile }: {
  ...
  const netEarnings = record.daily_wage_earned + (record.bonus_amount || 0) - (record.deduction_amount || 0);
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          ...
          <div>
            ...
            <p className="text-sm font-medium">{calculateWorkingHours(record.check_in, record.check_out)} ساعة</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          <div>
            ...
            <p className="text-sm font-medium text-green-600">{record.daily_wage_earned || 0} د.ل</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground mb-1">صافي الكسب</p>
            <p className="text-sm font-medium text-primary">{netEarnings.toFixed(2)} د.ل</p>
          </div>
        </div>
            ...
            {record.deduction_amount > 0 && (
              <div>
                ...
                {record.deduction_reason && (
                  <p className="text-muted-foreground text-xs">{record.deduction_reason}</p>
                )}
              </div>
            )}
        ...
        {record.early_departure && (
          <div className="mb-3">
            <Badge variant="destructive" className="text-xs">انصراف مبكر</Badge>
          </div>
        )}
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const { error } = await supabase
        .from('attendance')
        .update({
          deduction_amount: editFormData.deduction_amount ? parseFloat(editFormData.deduction_amount) : 0,
          bonus_amount: editFormData.bonus_amount ? parseFloat(editFormData.bonus_amount) : 0,
            ...
            return (
              <div key={employee.id} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <Button 
                  ...
                  className={`ml-2 ${hasAttendance ? "bg-gray-400" : "bg-green-600 hover:bg-green-700"}`}
                >
                  {hasAttendance ? (
                    <>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="max-w-sm mx-2 max-h-[90vh] overflow-y-auto" dir="rtl">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="deduction_amount" className="text-sm">مبلغ الخصم (د.ل)</Label>
                <Input
                  id="deduction_amount"
                  type="number"
                  value={editFormData.deduction_amount}
                  onChange={(e) => setEditFormData({...editFormData, deduction_amount: e.target.value})}
                  placeholder="0"
                  className="mt-1"
                />
              </div>
              ...
              <div>
                <Label htmlFor="deduction_reason" className="text-sm">سبب الخصم</Label>
                <Input
                  id="deduction_reason"
                  value={editFormData.deduction_reason}
                  onChange={(e) => setEditFormData({...editFormData, deduction_reason: e.target.value})}
              ...
              <div>
                <Label htmlFor="bonus_amount" className="text-sm">مبلغ المكافأة (د.ل)</Label>
                <Input
                  id="bonus_amount"
                  type="number"
                  value={editFormData.bonus_amount}
                  onChange={(e) => setEditFormData({...editFormData, bonus_amount: e.target.value})}
                  placeholder="0"
                  className="mt-1"
                />
              </div>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => setEditFormData({...editFormData, early_departure: e.target.checked})}
                  className="rounded"
                />
                <Label htmlFor="early_departure" className="text-sm">انصراف مبكر</Label>
              </div>
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button variant="outline" onClick={() => setIsEditDialogOpen(false)} className="flex-1 text-sm">
                  إلغاء
                </Button>
              </div>
            </div>
